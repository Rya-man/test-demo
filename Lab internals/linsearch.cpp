...
int helper(vi&nums,int a,int i)
{
    if(i>=nums.size())
    return -1;
    if(nums[i]==a)
    return i;
    return helper(nums,a,i+1);
}
pair<int,double> linsearch(vi&nums,int a)
{
    clock_t a1 =clock();
    auto loc =helper(nums,a,0);
    clock_t a2 =clock();
    return {loc,(double)a2-a1/CLOCKS_PER_SEC*1000};
}
void result(){
    int n;
    cin>>n;
